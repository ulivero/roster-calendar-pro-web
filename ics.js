...
function buildICSAndroid(events) {
  ...
  const lines = [
    'BEGIN:VCALENDAR',
    'VERSION:2.0',
    'PRODID:-//Roster Calendar Pro//AR Roster Android//ES',
    'CALSCALE:GREGORIAN',
    'METHOD:PUBLISH',
    'X-WR-CALNAME:AR Roster',
  ...
  for (const ev of events) {
    ...
    lines.push(`DTSTAMP:${now}`);
    lines.push(`LAST-MODIFIED:${now}`);
    lines.push(`DTSTART;TZID=${tzid}:${toLocalICS(ev.start)}`);
    lines.push(`DTEND;TZID=${tzid}:${toLocalICS(ev.end)}`);
    lines.push(`SUMMARY:${escapeICS(ev.title)}`);
    if (ev.location) lines.push(`LOCATION:${escapeICS(ev.location)}`);
    if (ev.description) lines.push(`DESCRIPTION:${escapeICS(ev.description)}`);
    lines.push('END:VEVENT');
...
window.RosterICS = { buildICS, buildICSAndroid };
